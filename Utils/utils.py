...
def setup_ui():
    ...
    ruta_css = rutas.resolver_ruta("static", "styles.css")
    ruta_logo = rutas.resolver_ruta("Img", "logo_nutresa.png")

    load_css("static/styles.css")
    # Cargar imagen como base64 para incrustarla en HTML
    image = Image.open(ruta_logo).resize((250, 250))
...
def calcular_venta(df, dict_cols):
    ...
    """
    # Convertir la columna original también
    df[dict_cols["Precio de venta"]] = df[dict_cols["Precio de venta"]].astype(float)
...
def procesar_insumo(df_insumo, porcentaje_crecimiento, dict_cols, cols_finales):
    """
    ...
    df_select = Seleccionar_columnas_pd(df=df, cols_elegidas=cols_finales)
    return df_select
